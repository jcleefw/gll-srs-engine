...
import { updateMasteryState, getNewlyMasteredIds } from '../../engine/session.js';
...
describe('getNewlyMasteredIds', () => {
  it('reports newly mastered word when mastery crosses threshold', () => {
    const prevState = makeState({ w1: { mastery: 0 } });
    const nextState = makeState({ w1: { mastery: 1 } });

    const newlyMastered = getNewlyMasteredIds(prevState, nextState, ['w1'], 1);
    expect(newlyMastered).toEqual(['w1']);
  });

  it('does not report a word already mastered in prevState', () => {
    const prevState = makeState({ w1: { mastery: 1 } });
    const nextState = makeState({ w1: { mastery: 2 } });

    const newlyMastered = getNewlyMasteredIds(prevState, nextState, ['w1'], 1);
    expect(newlyMastered).toEqual([]);
  });

  it('reports multiple newly mastered words', () => {
    const prevState = makeState({ w1: { mastery: 0 }, w2: { mastery: 0 } });
    const nextState = makeState({ w1: { mastery: 1 }, w2: { mastery: 1 } });

    const newlyMastered = getNewlyMasteredIds(prevState, nextState, ['w1', 'w2'], 1);
    expect(newlyMastered).toContain('w1');
    expect(newlyMastered).toContain('w2');
  });

  it('handles words not present in nextState', () => {
    const prevState = makeState({ w1: { mastery: 0 } });
