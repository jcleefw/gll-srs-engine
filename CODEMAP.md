...
| `RULES.md` | Constraints for this package: pure engine only, no persistence/I/O, no storage-layer import, minimal runtime `dependencies` |
| `CHANGELOG.md` | Release history in Keep a Changelog format, versioned per SemVer |
| `MUTATION-LOG.md` | Durable trend log of `pnpm test:mutation` runs (score, killed/timeout/survived counts) — the `reports/mutation/` HTML/JSON output itself is gitignored |
