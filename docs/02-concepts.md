...
Retirement check: `ws.mastery >= masteryThreshold`. `masteryThreshold` is a caller-supplied config value, not an engine-level default — the value actually in effect is whatever the host configures (e.g. a server's `FIXED_SYSTEM.masteryThreshold`).
...
A sentence becomes eligible when all its words have `WordState.seen >= minSeenForSentence`. `minSeenForSentence` is a caller-supplied config value (e.g. a server's `FIXED_SYSTEM.sentenceScheduling.minSeenForSentence`), not a fixed engine default. Mastery is not required. The composer produces 3 directions:
