...
| `05-data-pipeline.md` | Developers | Conversation JSON → ingestion → DB → query layer → engine inputs; what the engine receives (`words`, `pool`, `sentenceContexts`, `config`) vs what the caller/host is responsible for |

**Note:** `README.md`'s "Related files" section references `src/learn/index.ts`
and `src/learn/{types,engine}/` paths (post-`learn/` layer split) and does not
list `05-data-pipeline.md` in its own index.
