...
Interactive by default: it prompts for a deck, then for each answer. Requires a
real terminal — piping stdin fails on `setRawMode`.

To run unattended, set `AUTO_MODE = true` in `demo/config.ts`. That auto-answers
every question and skips the deck prompt. It is a source constant, not an
...
| `AUTO_MODE` | boolean | When `true`, auto-answers every question and skips the deck prompt (uses the first deck). Edit this constant in `demo/config.ts` — it is not read from the environment. |
